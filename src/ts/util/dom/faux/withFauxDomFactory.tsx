...
import {FauxElement, FauxElementClass} from "./FauxElement";

interface InjectedFauxDom {
    
    connect<K extends ElementTag>(node: string, name: string, discardNode?: boolean): FauxElement<ElementByTag<K>>;
...
const InjectedFauxDom = {
    ...
    new<P>(FauxElement: FauxElementClass, setState: SetState<P, FauxDomState>): FauxDomDelegate {
        
        const connectedFauxDom: Map<string, FauxElement<any>> = new Map();
        let animateUntil = 0;
        let drawTimeout = 0;
        ...
        const connect = <K extends ElementTag>(node: K, name: string, discardNode: boolean = false): FauxElement<ElementByTag<K>> => {
...
export const withFauxDomFactory = function(FauxElement: FauxElementClass) {

...
export const CachedGraph = function({data, filter, order, color}: CachedGraphProps) {
    ...
    const updateGraphData = (): GraphData => VariableAreaStack({
    ...
    let graphData: GraphData = updateGraphData();
    ...
    let graph: Graph = updateGraph();
    
    const updateNode = (): ReactNode => graph({
        color: (e, i) => color(order(e, i)),
    });
    let node: ReactNode = updateNode();
    
    return (props: CachedGraphProps) => {
        const sameGraphData = props.data === data && props.filter === filter;
        if (!sameGraphData) {
            data = props.data;
            filter = props.filter;
